...
class Game_test extends QP_Test_Controller
{
  ...
  public function score()
  {
    ...
    $this->unit->run($test, 0, 'Initial score is 0');

    $this->_game->score(10);
    $test = $this->_game->score();
    $this->unit->run($test, 10, 'Set score as 10');

    $this->_game->score(20);
    $test = $this->_game->score();
    $this->unit->run($test, 30, 'Increment score by 20');

    $this->_game->score(40);
    $test = $this->_game->score();
    $this->unit->run($test, 70, 'Increment score by 40');

    $this->_game->score(0, FALSE);
    $test = $this->_game->score();
    $this->unit->run($test, 0, 'Reset score to 0');
  ...
  public function level()
  {
    ...
    $this->unit->run($test, 1, 'Initial level is 1');

    $this->_game->level(TRUE);
    $test = $this->_game->level();
    $this->unit->run($test, 2, 'Increment level to 2');

    $this->_game->level(TRUE);
    $test = $this->_game->level();
    $this->unit->run($test, 3, 'Increment level to 3');
